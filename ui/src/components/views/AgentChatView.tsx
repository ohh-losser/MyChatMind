...
const AgentChatView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 如果是从新会话导航过来的，立即显示用户消息并运行 Agent
    if (state?.init && state?.initMessage && state?.agentId) {
      ...
      // 异步调用后端运行 Agent（不阻塞）
      runAgent(state.agentId, chatSessionId, state.initMessage).catch((error) => {
        console.error("运行 Agent 失败:", error);
        antdMessage.error("运行 Agent 失败");
      });
    }
  }, [chatSessionId, getChatMessages, state]);
  ...
  const handleSendMessage = async (value: string | { text: string }) => {
    ...
    if (!chatSessionId) {
      ...
      // 异步调用后端运行 Agent（不阻塞）
      runAgent(agentId ?? "", chatSessionId, message).catch((error) => {
        console.error("运行 Agent 失败:", error);
        antdMessage.error("运行 Agent 失败");
      });
    }
